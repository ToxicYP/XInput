...
class Encoder(pl.LightningModule):
    def __init__(self, d_model: int, growth_rate: int, num_layers: int):
        ...
        # self.PAM = PAM_Module(d_model)
        # self.CAM = CAM_Module(d_model)
        # norm_layer = nn.BatchNorm2d
        # inter_channels = d_model
    ...
    def forward(
        ...
        # sa_feat = self.PAM(feature)
        # sc_feat = self.CAM(feature)
        
        # sa_conv = self.conv51(sa_feat)
        # sc_conv = self.conv52(sc_feat)
        # sa_conv = self.conv6(sa_conv)
        # sc_conv = self.conv6(sc_conv)
